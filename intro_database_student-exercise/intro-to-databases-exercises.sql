...
SELECT name, continent, population, gnp, lifeexpectancy FROM World.dbo.country WHERE lifeexpectancy > 80;
-- 9. The name and population of all cities in the USA with a population of greater than 1 million people (9 rows)
SELECT name, population FROM World.dbo.city WHERE countrycode ='USA' and population > 1000000;
-- 10. The name and population of all cities in China with a population of greater than 1 million people (35 rows)
SELECT name, population FROM World.dbo.city WHERE countrycode ='CHN' and population > 1000000; 
-- 11. The name and region of all countries in North or South America (51 rows)
SELECT name, region FROM World.dbo.country WHERE continent in ('North America', 'South America');
-- 12. The name, continent, and head of state of all countries whose form of government is a monarchy (43 rows)
SELECT name, continent, headofstate From World.dbo.country Where governmentform like ('%monarchy%');
-- 13. The name of all cities in the USA with a population between 1 million and 2 million people (6 rows) 
SELECT name FROM World.dbo.city WHERE countrycode = 'USA' and population > 1000000 and population < 2000000;
-- 14. The name and region of all countries in North or South America except for countries in the Caribbean (27 rows)
SELECT name, region FROM World.dbo.country WHERE continent in ('North America', 'South America') and region != 'Caribbean';
...
SELECT name, population, gnp FROM World.dbo.country where gnp > 1000000 and population < 100000000; 
-- 16. The name and population of all cities in Texas that have a population of greater than 1 million people (3 rows)
SELECT name, population FROM World.dbo.city WHERE district = 'Texas' and population > 1000000;
-- 17. The name and average life expectancy of all countries on the continent of Oceania (28 rows)
SELECT name, lifeexpectancy FROM World.dbo.country WHERE continent = 'Oceania';
...
SELECT name, lifeexpectancy FROM World.dbo.country WHERE continent = 'Oceania' and lifeexpectancy is not null;
...
SELECT name FROM World.dbo.country WHERE continent = 'Oceania' and lifeexpectancy is null;
...
SELECT name, continent, gnp, lifeexpectancy FROM World.dbo.country WHERE lifeexpectancy >= 70 and gnp > 1000000 and gnp < 100000000;
-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
SELECT gnp, population as 'population', (gnp/population) as percapitagnp
...
SELECT indepyear as 'indepyear', (2020 - indepyear) as yearsinceindep
FROM World.dbo.country
